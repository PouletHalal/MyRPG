...
static bool is_mouse_over(sfVector2i pos, entity_t *entity)
{
    ...
    if (pos.x >= bounds.left && pos.x <= bounds.left + bounds.width && pos.y >= bounds.top && pos.y <= bounds.top + bounds.height)
        return true;
    return false;
}
                             ...
                             entity_t *entity, int i)
{
    ...
    if (is_mouse_over(mouse_pos, &world->entity[items[i].id_in_world]) && !world->mouse_left_pressed && !world->mouse_right_pressed)
    {
        display_tooltips(
...
bool use_item(win_t *window, entity_t *player, entity_t *item, int i)
{
    if (!player->comp_input.mouse_left_down || !item->comp_item.type_mask ||
        !is_mouse_over(sfMouse_getPositionRenderWindow(window->window), item) || (item->comp_item.type_mask & ITEM_CONSUMABLE) != ITEM_CONSUMABLE)
        return false;
...
void item_events(win_t *window, world_t *world, entity_t *entity)
{
    ...
    for (int i = 0; i < entity->comp_inventory.size; i++)
    {
        if (item_higlighting(window, world, entity, i))
            return;
        if (drop_item(window, entity, &world->entity[entity->comp_inventory.items[i].id_in_world], i))
            return;
...
void manage_inv_slots(world_t *world, win_t *window, entity_t *entity)
{
    ...
    if (mouse->comp_mouse.item_picked) {
        sfSprite_setPosition(world->entity[items[mouse->comp_mouse.item_picked_i].id_in_world].comp_render.sprite, (sfVector2f){mouse_pos.x, mouse_pos.y});
    ...
    for (int i = 0; i < entity->comp_inventory.size; i++)
    {
        if (entity->comp_inventory.items[i].type_mask != 0)
        {
            if (is_mouse_over(mouse_pos, &world->entity[items[i].id_in_world]) && world->mouse_left_pressed)
            {
                ...
                sfSprite_setPosition(world->entity[items[i].id_in_world].comp_render.sprite, (sfVector2f){mouse_pos.x, mouse_pos.y});
                return;
            }
        }
    }
